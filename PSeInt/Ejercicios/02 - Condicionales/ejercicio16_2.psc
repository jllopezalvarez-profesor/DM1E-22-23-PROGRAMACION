...
Proceso CalcularCosteTelefono
	...
	Definir es_domingo Como Logico; // Así no tengo que convertir cada vez en mayúsculas
	Definir turno, domingo Como Caracter;
	...
	es_domingo <-  Mayusculas(domingo) = "S" ;
	
	// No preguntamos el turno si es domingo, así optimizamos consultas al usuario
	Si no es_domingo Entonces
		Escribir Sin Saltar "¿Qué turno: Mañana o Tarde? (M/T)?:";
		Leer turno;
	FinSi
	
	// La primera solución es directamente del blog, observa que aquí ponemos <=
	// ¿Por qué?
	Si tiempo<=5 Entonces
		coste<-tiempo*100;
	SiNo
		Si tiempo<=8 Entonces // más de 5 minutos, pero menos de 8
